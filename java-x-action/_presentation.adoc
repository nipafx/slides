...
* unnamed patterns (22: https://openjdk.org/jeps/456[JEP 456])
* string templates (23?: https://openjdk.org/jeps/465[JEP 465])
* `StructuredTaskScope`
...
////
TODO: Use this once code formatter can handle string temnplate syntax
```java
public static String pageName(Page page) {
	return switch (page) {
		case ErrorPage(URI url, _)
			 -> STR."💥 ERROR: \{url.getHost()}}";
		case ExternalPage(URI url, _)
			-> STR."💤 EXTERNAL: \{url.getHost()}";
		case GitHubIssuePage(_, _, _, int nr)
			-> STR."🐈 ISSUE #\{nr}";
